...
module.exports = {
  new: function userNew (username, password, recoveryMethod, email, callback) {
    ...
    if (username.trim().toLowerCase().indexOf('admin_') ||
        username.trim().toLowerCase().indexOf('terminal_'))
      return callback(newError("UserExists", 400));
  ...
  sendMoney: function userSendMoney (user, receiverName, value, callback) {
    ...
    if (isNaN(parseInt(value)) || !isFinite(value) || value < 1 || value % 1 != 0)
      ...
      db.getUserByName(receiverName, function (err, receiver) {
        if (err) callback(err);
        else if (!receiver) callback(newError("ReceiverNotFound", 400));
        else if (user.balance < value) callback(newError("BalanceInsufficient"));
        else {
          var transaction = {
          ...
          async.waterfall([
            ...
            if (err) callback(err);
            // Everything was good, return without error
            else callback(null);
          });
        }
      })
  ...
  changePassword: function userChangePassword (user, oldPassword, newPassword, callback) {
    auth.checkPassword(user, oldPassword, (err, valid) => {
      if (err) callback(err);
      else if (!valid) callback(newError("PasswordWrong", 400));
